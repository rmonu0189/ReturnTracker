...
- (void)animateView:(BOOL)isShow textField:(id)textField heightforkeyboard:(CGFloat)kheight {
	...
	if (isShow) {
        ...
        else
        {
            if ([textField isKindOfClass:[UITextField class]]) {
                ...
                if (rect.size.height - textPoint.y < kheight)
                    rect.origin.y = rect.size.height - textPoint.y - kheight + viewFrameY-50;
                else rect.origin.y = viewFrameY;
            }
	...
	else rect.origin.y = viewFrameY-70;
...
//Set up previousBarItem Or  nextBarItem Click Allow 
- (void)checkBarButton:(id)textField {
	int i = 0,j = 0;
...
- (void)textFieldDidBeginEditing:(UITextField *)textField {
    ...
    if([self.keyBoardDelegate respondsToSelector:@selector(keyborardDidShow)]){
        [self.keyBoardDelegate keyborardDidShow];
    }
    //Ran change--> for take defualt space
   // NSString *space=@"  "
    
   // [textField setText:@"   "];//
    //[textField setText:[space stringByAppendingFormat:@"%@",textField.text]];
}
...
- (void)textFieldDidEndEditing:(UITextField *)textField {
    //Ran Change--> if text field not contained any value then space also remove from it
    ...
    if (textField.tag==1) {
        [delegate doneBtnMethodOfReturnDuration];
    }
}
...
- (id)initWithControllerDelegate:(id <UIKeyboardViewControllerDelegate>)delegateObject withMainDelegate:(id)delegate{
	if (self = [super init]) {
        ...
        viewFrameY = objectView.frame.origin.y+70;
